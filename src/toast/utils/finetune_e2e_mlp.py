...
import fire
from datasets import DownloadConfig, VerificationMode, load_dataset, load_from_disk
...
from toast.modules.module import HFwrapper, MLPLinearisedEncoder, SkipModel
...
def finetune_e2e_mlp_run(
    ...
    grad_clip: float = 1.0,
    # Output
    results_file: str = "finetune_e2e_mlp.csv",
    # Logging
    use_wandb: bool = True,
    wandb_project: str = "toast",
):
    layers_to_approximate = _parse_list(layers_to_approximate)
    mlp_layers_to_linearize = _parse_list(mlp_layers_to_linearize)

    head_tag = "pretrained" if use_pretrained_head else classifier_type
    print(
        f"\n[E2E-MLP] Dataset={dataset_name} | Model={model_name} | "
        f"Skip={layers_to_approximate} | MLP={mlp_layers_to_linearize} | "
        f"Translator={translator_name} | Head={head_tag} | Seed={seed}"
    )
    ...
    run_name = (
        f"{dataset_name}|{model_name_slug}|skip={layers_to_approximate}"
        f"|mlp={mlp_layers_to_linearize}|{translator_name}|{head_tag}|s{seed}"
    )
    wandb_run = None
    if use_wandb:
        import wandb
    ...
    if dataset_name == "imagenet-1k":
        ...
        mnist_path = "/cluster/customapps/biomed/vogtlab/users/mwylie/toast/mnist_clean"
        data = load_from_disk(mnist_path)
    ...
    # Apply MLP linearization in-place, then hand the patched inner encoder to SkipModel
    mlp_encoder = MLPLinearisedEncoder(
        encoder, mlp_layers_to_linearize=mlp_layers_to_linearize
    ).to(device).eval()
    skip_encoder_base = mlp_encoder.encoder  # patches already applied in-place

    if needs_translator_fitting:
        print(f"Extracting {samples_to_extract} samples for translator fitting...")
        with torch.no_grad():
            all_layer_embeddings = extract_representations(
                encoder=skip_encoder_base,
    ...
    print(f"Building SkipModel with skip={layers_to_approximate}, mlp={mlp_layers_to_linearize}...")
    ...
    if use_pretrained_head:
        ...
        scheduler = CosineAnnealingLR(
            ...
            eta_min=(encoder_lr if encoder_lr is not None else lr) * 0.01,
        )
    ...
    columns = [
        ...
        "approx_layer", "mlp_linearize", "num_layers", "original_accuracy",
        "accuracy", "best_accuracy", "delta_acc", "num_samples", "mode",
    ]
    ...
    mlp_str = str(mlp_layers_to_linearize)
    original_accuracy = 0.0
    if layers_to_approximate:
        filtered = results_df[
            (results_df["approx_layer"] == "[]")
            & (results_df["mlp_linearize"] == mlp_str)
...
if __name__ == "__main__":
    fire.Fire(finetune_e2e_mlp_run)
